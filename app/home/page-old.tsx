...
export default function HomePage() {
  ...
  // Legacy wallet creation function (kept for compatibility)
  const createLegacyWallet = async () => {
    if (!user) return
      ...
      try {
        const response = await fetch('/api/wallet/create', {
          ...
          headers: {
            ...
            'Authorization': `Bearer ${localStorage.getItem('prepx_token')}`,
          },
          body: JSON.stringify({
            userId: user.id,
        ...
        if (response.ok) {
          ...
          setLocalHyperliquidWalletAddress(data.walletAddress)
          setHyperliquidWalletAddress(data.walletAddress)
          setHyperliquidConnected(true)
          setIsSetupComplete(true)
          
          // Save to localStorage
          localStorage.setItem('hyperliquidWalletAddress', data.walletAddress)
        ...
        setLocalHyperliquidWalletAddress(mockWalletAddress)
        setHyperliquidWalletAddress(mockWalletAddress)
        setHyperliquidConnected(true)
        setIsSetupComplete(true)
        
        // Save to localStorage
        localStorage.setItem('hyperliquidWalletAddress', mockWalletAddress)
  ...
  useEffect(() => {
    ...
    if (isSuperApp && hasUser && ethereumAddress && ethereumPrivateKey) {
      ...
      } else if (user && !user.hasWallet) {
        // New user without wallet, create one automatically
        createWallet()
      }
    }
  }, [isSuperApp, hasUser, ethereumAddress, ethereumPrivateKey, setHyperliquidWalletAddress, refreshBalances, user])
          ...
          <Card className="bg-[#1a1a1a] border-[#262626] p-4 sm:p-6 rounded-2xl">
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white">Trading Ready</h3>
            </div>
            ...
            <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-[#27c47d] font-medium text-sm">Automatically Connected</span>
                </div>
