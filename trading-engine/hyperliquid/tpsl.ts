...
import { priceFeeds, fetchPrice } from "./hyperliquid";
// Simple logTrade function to replace the deleted tradeLogger
function logTrade(tradeData: any) {
...
import { client } from "./hyperliquid"; // ✅ use your instantiated client
...
export async function checkAndCloseForTP({
  ...
  const positions = (state?.assetPositions ?? []).map(p => ({
    ...
    side: "side" in p?.position ? (p.position.side as string).toLowerCase() : "unknown",
  ...
  for (const pos of positions) {
    if (!pos.coin || !pos.entryPx || !pos.szi) continue;

    const symbol = Object.keys(priceFeeds).find((s) => pos.coin.includes(s));
    if (!symbol) continue;
    ...
    if (shouldClose) {
      const result: "win" | "loss" | "liquidated" = "liquidated";
      ...
      try {
        ...
        winRateTracker.recordTradeLiquidated(symbol, mark, pnl);
      } catch (e) {
      ...
      recordAIPOS(
        ...
        stored.signalScore,
        stored.marketRegime,
        0, // No TP when profit goal is active
        ...
        stored.breakevenActivated,
        stored.trailingTPPct,
        stored.divergenceScore,
        stored.tradeType,
        stored.note,
        stored.phase,
        stored.highestFav,
