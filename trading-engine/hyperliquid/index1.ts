  ...
  const profitGoal = await new Promise<number>((resolve) => {
    rl.question('Enter your profit goal ($): ', (answer) => {
      ...
      if (isNaN(goal) || goal <= 0) {
        console.log('❌ Invalid profit goal. Using default: $200');
        resolve(200);
      } else {
        resolve(goal);
      }
...
async function runSession() {
  ...
  enhancedLogger.startSession(sessionId, {
    ...
    maxPositions: maxPerSession
  });
  
  // Send Telegram notification for session start
  logToTelegram('SESSION', `🚀 Session started | Budget: $${maxBudget} | Goal: $${profitGoal} | Max Positions: ${maxPerSession}`);
  ...
  for (let cycle = 0; cycle < MAX_CYCLES; cycle++) {
    ...
    if (pnl != null) {
      ...
      if (pnl >= profitGoal) {
        ...
        if (closedCount > 0) {
          ...
          // Send Telegram notification for profit goal achievement
    ...
    if (openCount === 0 && slotsLeft === maxPerSession && sessionCount > 0) {
...
async function runBotWithRestart() {
  ...
  while (totalRestartCount < MAX_RESTARTS) {
    try {
      ...
      // Send Telegram notification for bot start/restart
      logToTelegram('BOT', `🔄 Starting session #${sessionCount} (restart #${totalRestartCount})`);
      ...
      if (result.shouldRestart) {
        ...
        // Send Telegram notification for restart
        ...
        logToTelegram('BOT', `✅ Bot completed normally after ${sessionCount} sessions`);
      ...
      if (err instanceof Error) {
        enhancedLogger.logError(err, `session_${sessionCount}`);
      }
      
      // Send Telegram notification for fatal error
      logToTelegram('ERROR', `💥 Fatal error in session #${sessionCount}: ${errorMessage}`);
