...
exports.priceFeeds = exports.client = exports.publicClient = exports.transport = exports.account = void 0;
...
exports.recordLiquidatedTrades = recordLiquidatedTrades;
exports.recordExistingPositionsAsTrades = recordExistingPositionsAsTrades;
...
const symbolResolver_1 = require("./symbols/symbolResolver");
...
async function recordTradeInWinRateTracker(symbol, exitPrice, size, reason) {
    try {
        ...
        if (stored && exitPrice > 0) {
            ...
            const side = stored.side || (positionSize > 0 ? 'LONG' : 'SHORT');
            ...
            if (reason.includes('liquidated')) {
                ...
                winRateTracker_1.winRateTracker.recordTradeLiquidated(symbol, exitPrice, pnl);
            }
            else if (pnl > 0) {
                console.log(`✅ [WIN_RATE] Recording WIN for ${symbol} with PnL $${pnl.toFixed(2)}`);
                winRateTracker_1.winRateTracker.recordTradeWin(symbol, exitPrice, pnl, reason);
            }
            else {
                console.log(`❌ [WIN_RATE] Recording LOSS for ${symbol} with PnL $${pnl.toFixed(2)}`);
                winRateTracker_1.winRateTracker.recordTradeLoss(symbol, exitPrice, pnl, reason);
            }
        }
        else {
            // Fallback: Try to get entry data from win rate tracker if available
            const openTrade = winRateTracker_1.winRateTracker.getOpenTrade(symbol);
...
function getAccount() {
    ...
    if (!_account) {
        _account = (0, accounts_1.privateKeyToAccount)(rawKey);
...
exports.account = {
    ...
    signMessage: (message) => getAccount().signMessage(message),
...
function parseOrderStatus(status, requestedSize) {
...
async function waitForOrderFill(orderId, maxWaitTime = 30000) {
    const startTime = Date.now();
    const checkInterval = 2000; // Check every 2 seconds
    while (Date.now() - startTime < maxWaitTime) {
        try {
            // Get order status from Hyperliquid
            const orderStatus = await retry(() => exports.publicClient.orderStatus({ oid: parseInt(orderId), user: exports.account.address }));
...
async function fetchPrice(symbol) {
    if (!symbol)
        return 0;
    try {
        const hlSymbol = (0, symbolResolver_1.resolveToHLName)(symbol);
        const book = await retry(() => exports.publicClient.l2Book({ coin: hlSymbol }));
...
async function getPositions() {
    try {
        const state = await retry(() => exports.publicClient.clearinghouseState({ user: exports.account.address }));
...
async function closePosition(symbol, pos, reason, price) {
    try {
        const meta = await retry(() => exports.publicClient.meta());
        const hlSymbol = (0, symbolResolver_1.resolveToHLName)(symbol);
        ...
        try {
            ...
            const getDecimalPlaces = (price) => {
                if (price >= 1000)
                    return 1; // For high-priced tokens like MKR, BTC
                if (price >= 100)
                    return 2; // For medium-priced tokens
                if (price >= 10)
                    return 3; // For lower-priced tokens
...
async function closeAllPositions() {
    try {
        ...
        winRateTracker_1.winRateTracker.logStats();
        return {
            success: true,
...
async function recordLiquidatedTrades() {
    try {
        const positions = await getPositions();
        ...
        for (const pos of positions) {
            ...
            const openTrade = winRateTracker_1.winRateTracker.getOpenTrade(pos.coin);
            if (!openTrade) {
            ...
            winRateTracker_1.winRateTracker.recordTradeLiquidated(pos.coin, mark, pnl);
        }
    }
    catch (err) {
...
async function recordExistingPositionsAsTrades() {
    try {
        const positions = await getPositions();
        ...
        for (const pos of positions) {
            ...
            // Check if this trade is already recorded
            const existingTrade = winRateTracker_1.winRateTracker.getOpenTrade(pos.coin);
            if (existingTrade) {
            ...
            winRateTracker_1.winRateTracker.recordTradeOpen(pos.coin, entry, Math.abs(size), side);
        }
    }
    catch (err) {
...
async function runSignalCheckAndOpen({ symbol, perPositionBudget, regimeOverride, leverage }) {
    const ohlcv4h = await (0, binanceHistorical_1.getCachedOHLCV)(symbol, "4h", 300);
    const ohlcv6h = await (0, binanceHistorical_1.getCachedOHLCV)(symbol, "6h", 300);
    ...
    const { regime: guessedRegime, confidence: regimeConfidence } = regimeOverride ? { regime: regimeOverride, confidence: 1.0 } : await (0, regime_1.guessMarketRegime)(symbol, ohlcv4h, ohlcv6h);
    console.log(`🧠 Regime detected for ${symbol}: ${guessedRegime} (${(regimeConfidence * 100).toFixed(1)}%)`);
    const result = await (0, strategyEngine_1.evaluateSignalOnly)(symbol, ohlcv4h, {
    ...
    const safeBudget = new decimal_js_1.default(perPositionBudget);
    ...
    const meta = await exports.publicClient.meta();
    const hlSymbol = (0, symbolResolver_1.resolveToHLName)(symbol);
    const assetIndex = meta.universe.findIndex((a) => a.name === hlSymbol);
    ...
    try {
        ...
        if (entryPrice > 0 && positionSize > 0) {
            winRateTracker_1.winRateTracker.recordTradeOpen(symbol, entryPrice, positionSize, side);
        }
    }
    catch (trackerError) {
        console.warn(`⚠️ Failed to record trade opening for ${symbol}:`, trackerError);
    }
    (0, aiStorage_1.recordAIPOS)(symbol, orderResult.avgFillPrice || roundedPrice, orderResult.orderId.toString(), signalScore, guessedRegime, tp, sl, rsi, macdHist, emaSlope, atrPct, leverage, atr, adx, adxSlope, volumePct, parseFloat(safeBudget.toFixed(6)), false, 0, divergenceScore, "anticipation", result.reason, "init", undefined, undefined, undefined, triggeredBy, entryReason, undefined, // extraTP
    undefined, // extraSL
    rrr, // extraRRR
...
exports.client = exports.publicClient;
exports.priceFeeds = {
    BTC: 'BTC/USDT',
    ETH: 'ETH/USDT',
    BNB: 'BNB/USDT',
