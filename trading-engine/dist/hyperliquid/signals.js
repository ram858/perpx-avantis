...
exports.calculateSignalScore = exports.TradeMemory = void 0;
...
function calculateSignalScoreWeighted({ rsi, macdHist, emaSlope, atrPct, adx, divergenceScore = 0 }, lastPrice) {
    const rsiComponent = rsi > 70 || rsi < 30 ? 1 : rsi > 55 || rsi < 45 ? 0.8 : 0.5;
    const macdComponent = Math.abs(macdHist) / lastPrice > 0.0002 ? 1 : 0.3; // Normalize
    ...
    return Math.min(1, (1.2 * rsiComponent + 0.5 * macdComponent + 1.5 * emaSlopeComponent + 1.0 * atrComponent + 1.0 * adxComponent + 0.3 * divergenceComponent) / 5.5);
}
...
function calculateVolumePercent(volume) {
    ...
    const avgVol = volume.slice(-50).reduce((a, b) => a + b, 0) / 50; // Use SMA
...
function checkSignals(symbol, ohlcv, configOverride, forcedRegime) {
    ...
    const signalScore = calculateSignalScoreWeighted({
        ...
        divergenceScore: indicatorInputs.divergenceScore ?? 0
    }, lastPrice);
...
function calculateMOS({ ohlcv, mtfSlopes }) {
    const close = ohlcv.close;
    const volume = ohlcv.volume;
    const high = ohlcv.high;
    const low = ohlcv.low;
    const lastPrice = close.at(-1);
    const trendSlopePct = linearSlope(close.slice(-20)) / lastPrice * 100; // Normalize
    const adxArr = technicalindicators_1.ADX.calculate({ high, low, close, period: 14 });
    const adx = adxArr.length ? adxArr.at(-1).adx : 20;
    const TrendBias = trendSlopePct * (adx / 50);
    const macdArr = technicalindicators_1.MACD.calculate({
        values: close,
        fastPeriod: 12,
        slowPeriod: 26,
    ...
    const macdPrev = macdArr.at(-2);
    const macdDiffPct = ((macd?.histogram ?? 0) - (macdPrev?.histogram ?? 0)) / lastPrice;
    const rsiArr = technicalindicators_1.RSI.calculate({ values: close, period: 14 });
    const rsiSlope = linearSlope(rsiArr.slice(-14)) / 50; // Normalize
    const MomentumImpulse = macdDiffPct + rsiSlope;
    const currentVolume = volume.at(-1);
    const avgVolume = volume.slice(-12).reduce((a, b) => a + b, 0) / 12;
    const VolumeDelta = (currentVolume / avgVolume) - 1;
    const atrArr = technicalindicators_1.ATR.calculate({ high, low, close, period: 14 });
    const atr = atrArr.at(-1) ?? 0;
    const atrPrev = atrArr.at(-2) ?? atr;
    const VolatilityClarity = 1 - Math.abs(atr - atrPrev) / (atr || 1);
    const MultiTimeframeAlignment = new Set(mtfSlopes.map(s => Math.sign(s / lastPrice * 100))).size === 1 ? 1 : -1;
    return Number((0.35 * TrendBias + 0.25 * MomentumImpulse + 0.2 * VolumeDelta + 0.1 * VolatilityClarity + 0.1 * MultiTimeframeAlignment).toFixed(4));
}
